            ...
            if not result.stderr:
                ...
                if group_id:
                    ...
                    if not result_1.stderr:
                        res = result_1.stdout
                        # construct file_name
                        if exe_info.architecture_id == 5 :
                            org_id = exe_info.organization
                            linuxfile(group_id,org_id)
                            file_name = f"InfraonRemoteAgentLinux-{exe_info.organization}.sh"
                        elif exe_info.architecture_id == 6 :
                            org_id = exe_info.organization
                            macfile(group_id,org_id)
                            file_name = f"InfraonRemoteAgentMacOS-{exe_info.organization}.sh"
                        else:
