...
def powcheck(group_id,device_id):
    print(">>>>>>>>>>>>>>>>>>>>>",device_id)
    try:
        command = f"node meshctrl listdevices --id {group_id} --filter [{device_id}] --url {WSS_URL} --loginuser {MESH_CENTRAL_USERNAME} --loginpass {MESH_CENTRAL_PASSWORD} "
        print(command)
        ...
        state=state[1:2]
        print(state)
        logger.info(">>>>>>>>>>>>>>>check power<<<<<<<<<<<<<< %s" %(state))
    except subprocess.CalledProcessError as e:
        logger.exception(">>>>>>>>>>>>>>>check power<<<<<<<<<<<<<< %s" %(e,state))
    return int(state)